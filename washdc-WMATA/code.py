...
if not DEBUG:
    large_font = bitmap_font.load_font("fonts/helvR14.bdf") #"fonts/Arial-12.bdf"
            ...
            if train["ALERT"] is True and bullet_alert_flag is True:
                if train["FLASH"]:
                    ...
                    if (now >= train["PREV_TIME"] + train["FLASH_ON"]):
                        self.prev_time = now
                        wmata_bullets[0,row] = bullet_index[train["Line"]]
...
while True:
    ...
    if subway_refresh is None or time.monotonic() > subway_refresh + 18:
