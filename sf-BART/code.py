...
# Tiles in /img/bart_sheet.bmp (8x8, two per row). Colors are BART's official line colors.
bullet_index = {"RED": 0,
                "ORANGE": 1,
                "BLUE": 2,
                "GREEN": 3,
                "YELLOW": 4,
                "GREY": 5,     # Oakland Airport connector (publishes no real-time data)
                "ALERT": 6,
                "BART": 7,     # neutral ring: unknown line, no service, errors
                "STARTUP_TOP": 8,     # lowercase b, shown in the top row until the first fetch
                "STARTUP_BOTTOM": 9,  # train nose, shown in the bottom row until the first fetch
                "BLANK": 15}

ROOT_CERT_PATH = "/gtsr4.pem"  # GTS Root R4, the root api.bart.gov's certificate chains to; see README
USER_AGENT = "arrivals-board/1.0 (MatrixPortal S3; CircuitPython)"
ARRIVALS_REFRESH = 15   # seconds; BART regenerates its estimates roughly every 15 s
NIGHT_START = 20        # from this hour (local time) all text is drawn in NIGHT_COLOR...
NIGHT_END = 6           # ...until this hour
ADVISORY_REFRESH = 120  # seconds between service advisory fetches (also the retry interval after a failure)
ADVISORY_MAX_AGE = 1800  # seconds; advisories we could not refresh for this long are dropped


def bullet(line):
    """Sprite index for a line color, falling back to the neutral ring for anything new."""
    return bullet_index.get(line, bullet_index["BART"])


class FeedError(Exception):
    """The API answered, but not with usable data (bad key, bad station, Cloudflare page)."""
...
def is_night(hour=None):
    """True between NIGHT_START and NIGHT_END, using the board clock (local time once synced)."""
    if hour is None:
        hour = time.localtime()[3]
    return hour >= NIGHT_START or hour < NIGHT_END


def text_color(day_color):
    """The color a text label should use right now."""
...
bart_sheet, bart_palette = imageload.load("/img/bart_sheet.bmp",
                                          bitmap=displayio.Bitmap,
                                          palette=displayio.Palette)
bart_bullets = displayio.TileGrid(bart_sheet,
                                  pixel_shader=bart_palette,
                                  width=1,
                                  height=2,
                                  tile_width=8,
                                  tile_height=8,
                                  default_tile=bullet_index["BART"])
bart_bullets.x = 24
bart_bullets.y = y_center - 1
bart_bullets[0, 0] = bullet_index["STARTUP_TOP"]
bart_bullets[0, 1] = bullet_index["STARTUP_BOTTOM"]

arrivals_north_bullets = displayio.TileGrid(bart_sheet,
                                            pixel_shader=bart_palette,
                                            width=1,
                                            height=4,
                                            tile_width=8,
...
arrivals_south_bullets = displayio.TileGrid(bart_sheet,
                                            pixel_shader=bart_palette,
                                            width=1,
                                            height=4,
                                            tile_width=8,
...
class Atmosphere:
    ...
    def update_display(self):
        weather_label.color = text_color(color[4])
        aqi_label.color = text_color(color[4])
...
class Arrivals:
    """Fetches BART's real-time departures straight from api.bart.gov (no proxy)."""

    def __init__(self):
        self.station = secrets["bart_station"].strip().upper()
        self.key = (secrets.get("bart_key") or "").strip() or bart.PUBLIC_KEY
        self.lines = bart.parse_lines(secrets.get("bart_lines", ""))
        # CircuitPython str has no .title(), so normalize with a lookup
        ...
        self.pool = None
        self.root_cert = None
        self.sessions = []  # requests sessions to try in order; whichever works is moved to the front
        self.advisories = []
        self.advisory_refresh = None  # when advisories were last requested
        self.advisory_fetched = None  # when advisories were last successfully parsed
    ...
    def wifi_lost_message(self):
        ...
        for row in range(2):
            bart_bullets[0, row] = bullet_index["BART"]

    def _open_sessions(self):
        """One session that trusts the shipped GTS Root R4 (works on every CircuitPython
        release), plus one that uses the firmware's certificate bundle as a fallback."""
        network.connect()
        ...
        if root_cert:
            ...
            sessions.append(adafruit_requests.Session(self.pool, context, session_id="bart-root"))
        sessions.append(adafruit_requests.Session(self.pool, ssl.create_default_context(), session_id="bart-bundle"))
        self.sessions = sessions

    def _reset_sessions(self):
        """Close every BART socket and forget the sessions; the next fetch rebuilds them.

        adafruit_requests can leave a socket registered but unusable after a failure while
        reading a response, and every later request on that session then fails immediately."""
        try:
            adafruit_connection_manager.connection_manager_close_all(self.pool, release_references=True)
        except Exception as e:
            if DEBUG:
                print("closing BART sockets failed:", e)
        self.sessions = []
    ...
    def _get_json(self, session, url):
        ...
        try:
            ...
            if "json" not in content_type:
                # a Cloudflare challenge or outage page arrives as text/html; drain it so the
                # keep-alive socket is clean for the next request
                try:
                    response.content
                except Exception:
                    pass
                raise FeedError("HTTP {} {}".format(response.status_code, content_type))
            data = response.json()
        finally:
            response.close()
        ...
        if response.status_code != 200:
            raise FeedError("HTTP {}".format(response.status_code))
        return data
    ...
    def _fetch_json(self, url):
        ...
        for index, session in enumerate(self.sessions):
            try:
                ...
                # socket or TLS trouble: try the next session
                last_error = e
                wedged = wedged or isinstance(e, RuntimeError)
                if DEBUG:
                    print("BART fetch failed on session", index, ":", e)
                continue
            if index:
                self.sessions.insert(0, self.sessions.pop(index))
            break
        if wedged or data is None:
    ...
    def api_call(self):
        if DEBUG:
            print("\n=== Calling BART etd API ===")
            print(f"URL: {self.etd_url}")

        try:
            etd = self._fetch_json(self.etd_url)
            now = time.monotonic()
            if self.advisory_refresh is None or now > self.advisory_refresh + ADVISORY_REFRESH:
                self.advisory_refresh = now  # retry no sooner than ADVISORY_REFRESH even after a failure
                try:
                    self.advisories = bart.parse_bsa(self._fetch_json(self.bsa_url))
                    self.advisory_fetched = now
                except Exception as e:
                    # advisories are a nice-to-have; keep the last known list for a while
                    print("BART advisories error:", e)
            if self.advisories and (self.advisory_fetched is None or now > self.advisory_fetched + ADVISORY_MAX_AGE):
                print("Dropping BART advisories that could not be refreshed")
                self.advisories = []
            arrival_data = bart.build_arrivals(etd, self.advisories, self.lines)
            if DEBUG:
                print("API Call Successful!")
                print(arrival_data)

        except Exception as e:
            print("BART arrivals error:", e)
            return None
        finally:
            gc.collect()
    ...
    def update_board(self, arrival_data):
        ...
        board_color = text_color(color[4])
        arrivals_row_nums.color = board_color
        arrivals_north_label.color = board_color
        arrivals_south_label.color = board_color

        if arrival_data is None:
            arrivals_north_label.text = "err\nerr\nerr\nerr"
            arrivals_south_label.text = "err\nerr\nerr\nerr"
            for i in range(4):
                arrivals_north_bullets[0, i] = bullet_index["BART"]
                arrivals_south_bullets[0, i] = bullet_index["BART"]
        ...
        for direction in self.directions:
            ...
            for i in range(rows):
                ...
                if arrival_time == 0:
                    arrival_time = "Due"  # BART reports this as "Leaving"
                    if direction == "North":
    ...
    def update_display(self, arrival_data, bullet_alert_flag, now):
        ...
        arrival_label_1.color = text_color(color[4])
        arrival_label_2.color = text_color(color[4])
        ...
        for i in range(rows):
            ...
            # bart.py sets Alert when the line is named in a service advisory or the train is badly late
            ...
            if train["ALERT"] is True and bullet_alert_flag is True:
                if train["FLASH"]:
                    if (now >= train["PREV_TIME"] + train["FLASH_OFF"]):
                        self.prev_time = now
                        bart_bullets[0, row] = bullet_index["ALERT"]
                        self.alert_flash = False

                elif train["FLASH"] is False:
                    if (now >= train["PREV_TIME"] + train["FLASH_ON"]):
                        self.prev_time = now
                        bart_bullets[0, row] = bullet(train["Line"])
                        self.alert_flash = True
            else:
                bart_bullets[0, row] = bullet(train["Line"])

        if rows == 1:
            bart_bullets[0, 1] = bullet_index["BLANK"]
            arrival_label_2.text = ""

    def alert_signature(self, arrival_data, alert_text):
        """Changes when the advisory text changes or a displayed train's alert state changes,
...
def update_time(*, hours=None, minutes=None, show_colon=False):
    ...
    if hours is None:
        hours = now[3]
    clock_label.color = text_color(color[3])
...
default_group.append(bart_bullets)      # ELEMENT 2
...
while True:
    ...
    if event:
        ...
        if event.pressed and event.key_number == 1:  # DOWN button
            alert_scroll_label.color = text_color(color[4])
            display_alt_text()
